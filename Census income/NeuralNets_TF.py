...
def construct_feature_columns(input_features):
    ...
    categorical_workclass = tf.feature_column.indicator_column(tf.feature_column.categorical_column_with_identity('workclass', num_buckets=max(
        input_features['workclass'])+1))
    categorical_education = tf.feature_column.indicator_column(tf.feature_column.categorical_column_with_identity('education', num_buckets=max(
        input_features['education'])+1))
    categorical_marital_status = tf.feature_column.indicator_column(tf.feature_column.categorical_column_with_identity('marital-status', num_buckets=max(
        input_features['marital-status'])+1))
    categorical_occupation = tf.feature_column.indicator_column(tf.feature_column.categorical_column_with_identity('occupation', num_buckets=max(
        input_features['occupation'])+1))
    categorical_relationship = tf.feature_column.indicator_column(tf.feature_column.categorical_column_with_identity('relationship', num_buckets=max(
        input_features['relationship'])+1))
    categorical_race = tf.feature_column.indicator_column(tf.feature_column.categorical_column_with_identity('race', num_buckets=max(
        input_features['race'])+1))
    categorical_native_country = tf.feature_column.indicator_column(tf.feature_column.categorical_column_with_identity('native-country', num_buckets=max(
...
def main():
    ...
    test_predictions = np.array([item['class_ids'][0] for item in test_probabilities])
    test_probabilities = classifier.predict(input_fn=predict_testing_input_fn)
    ...
    ax = sns.heatmap(cm, cmap="bone_r", annot=True, fmt="d", xticklabels=True,yticklabels=True)
    ax.set_aspect(1)
